...
- **Human-Gated Merges (`auto_merge: false`)**: Merging pull requests to the target branch (`main`) via `/gh-cli:pr-merge` or Phase P9 MUST ALWAYS gate on explicit human approval via the interactive `default_api:ask_question` tool before executing the merge.
