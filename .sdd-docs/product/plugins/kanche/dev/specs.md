...
## Capabilities
- `/dev:implement` - Incrementally apply codebase modifications according to task checklist.
...
1. Exposes `/dev:implement` slash command.
2. Registers subagent `coder` in `plugins/dev/plugin.json`.
3. Integrates with `/sdd:run` orchestrator for P4 (Build / Implementation) phase.
