...
- **Decision:** Enable `--delete-branch` flag as the default behavior for `/kanche:pr-merge` command execution, providing `--keep-branch` to override it.
...
- **Decision:** Automatically include the `--assignee "@me"` flag during `/kanche:pr-create` execution by default.
