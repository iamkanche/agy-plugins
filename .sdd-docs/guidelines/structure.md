...
## Repository Layout
- `plugins/`: Namespace directories for plugin bundles.
  - `kanche/`: Author namespace folder containing active plugin bundles.
...
- Each folder under `plugins/kanche/` (e.g. `git`, `gh-cli`, `sdd`) must remain fully self-contained and modular.
...
## Naming Conventions
- **Plugins:** Lowercase, kebab-case directory names under `plugins/kanche/` (e.g. `gh-cli`).
