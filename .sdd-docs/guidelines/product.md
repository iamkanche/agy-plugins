...
## Core Value
Provides pre-packaged workflow plugins (git control, GitHub CLI integrations, SDD lifecycle workflows) that extend agent capabilities with safe side-effect gates and group-prefixed slash command namespaces (`/kanche:<group>-<action>`).
...
- Group-prefixed slash commands across 7 domain groups: `sdd-*`, `git-*`, `gh-cli-pr-*`, `design-*`, `dev-*`, `planner-*`, and `qa-*`.
...
## Success Signals
- Google Antigravity agents can parse, register, and successfully run slash commands from these plugins in their local environments using group-prefixed names (e.g., `/kanche:sdd-run`, `/kanche:git-commit`, `/kanche:gh-cli-pr-create`).
