...
- **Never amend a pushed commit** (grounded in `.sdd-docs/product/plugins/kanche/git/rules/git-hard-rules.md`).
- **Never `git reset --hard` on shared work** (grounded in `.sdd-docs/product/plugins/kanche/git/rules/git-hard-rules.md`).
- **Never commit secrets** (such as `.env`, credentials, private keys, or tokens) (grounded in `.sdd-docs/product/plugins/kanche/git/rules/git-hard-rules.md` and `.sdd-docs/product/plugins/kanche/gh-cli/rules/gh-hard-rules.md`).
- **Never approve your own PR** (grounded in `.sdd-docs/product/plugins/kanche/gh-cli/rules/gh-hard-rules.md`).
- **Never force-merge or bypass status checks** (grounded in `.sdd-docs/product/plugins/kanche/gh-cli/rules/gh-hard-rules.md`).
- **Use COMMENT event by default** when reviewing a PR unless explicitly instructed (grounded in `.sdd-docs/product/plugins/kanche/gh-cli/rules/gh-hard-rules.md`).
