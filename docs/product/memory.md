...
- **Flattened Plugin Skill Structure**: All 38 skills in the `kanche` plugin use single-level flattened subdirectories under `plugins/kanche/skills/<skill-name>/SKILL.md` (e.g. `sdd-init`, `git-commit`, `gh-cli-pr-create`, `code-review`, `security-scan`) to ensure full compatibility with the `agy plugin install` CLI engine and AGY slash command discovery.
