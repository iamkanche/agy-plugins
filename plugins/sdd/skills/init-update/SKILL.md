...
5. **Persist (merge).** Write each returned doc back to `.sdd-docs/guidelines/{product,tech,structure,rules}.md`. Because the skill returns the already-merged full text, a full write is correct — but never discard hand-tuned sections the skill preserved. If a doc came back unchanged, still write it (a no-op is safe).
